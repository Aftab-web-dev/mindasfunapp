...
export const authApi = {
  postAuth: async function ({ query }: { query: { password: string, email: string } }) {
    ...
    return await axiosConfig.post(`Login/CheckLogin?${queryParams}`, null, { skipAuthRedirect: true })
  },
  updateProfile: async function ({ query }: { query: { password: string, username: string, empId: number | undefined} }) {
    ...
    return await axiosConfig.post(`Login/UpdateProfile?${queryParams}`, null, { skipAuthRedirect: true })
  }
}
