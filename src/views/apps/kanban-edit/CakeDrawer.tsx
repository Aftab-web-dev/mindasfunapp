...
export default function CakeDrawer({
  ...
  const cakeName = watch('name')
  const cakeWeight = watch('cake_weight')

  useEffect(() => {
    const selected = cakeOptions.find(c => c.name === cakeName)

    if (selected) {
      setValue('cake_per_kg', selected.price)
      setValue('cake_amount', selected.price * (cakeWeight || 1))
    }
  }, [cakeName, cakeWeight, setValue])
          ...
          <Controller
            ...
            render={({ field }) => (
              ...
              >
                ...
                {cakeOptions.map((item: any, index: any) => (
